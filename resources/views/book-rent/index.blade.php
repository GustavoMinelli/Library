    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex mb-3 justify-end">
                <x-primary-link-button href="{{ route('book-rents.create') }}">
                    {{ __('Novo Aluguel de Livro') }}
                    ...
                    <table class="table md:table-auto relative h-full mb-5 table-fixed w-full text-center border-solid">
                        <thead>
                            <tr class="border-solid border-0 border-b border-slate-700">
                                ...
                                <th class="">Cliente</th>
                                <th class="">Livro</th>
                                <th class="">Devolvido</th>
                                <th class="">Atrasado</th>
                                <th class="">Data de devolução</th>
                                <th>Ações</th>
                            </tr>
                        </thead>
                        ...
                        <tbody class="h-10">
                            ...
                            @foreach ($bookRents as $bookRent)
                                ...
                                <tr class="border-solid border-0 border-b border-slate-700 last:border-transparent">
                                    <td>{{ $bookRent->id }}</td>
                                    <td>{{ $bookRent->customer_name }}</td>
                                    <td>{{ $bookRent->book_title }}</td>
                                    <td>{{ $bookRent->is_returned ? 'Sim' : 'Não'}}</td>
                                    <td>{{ $bookRent->is_late ? 'Sim' : 'Não'}}</td>
                                    <td>{{ date('d/m/Y', strtotime($bookRent->expired_at)); }}</td>

                                    <td>
                                        <x-primary-link-button href="{{ url('/aluguel-de-livros/'.$bookRent->id.'/editar') }}">
                                            <i class="fas fa-edit"></i></x-primary-link-button>

                                        <x-danger-button
                                                x-data=""
                                                x-on:click.prevent="$dispatch('open-modal', 'confirm-book-rent-delete-{{ $bookRent->id }}')"
                                            ><i class="fas fa-trash"></i></x-danger-button>

                                            <x-modal name="confirm-book-rent-delete-{{ $bookRent->id }}">
                                                <form method="post" action="{{ url("aluguel-de-livros/$bookRent->id") }}" class="p-6">
